...
- (void)initValues
{
    DLog(@"%f  %f",self.bounds.size.width,self.bounds.size.height);
    //创建顶部可滑动的tab
...
- (void)buildUI
{
    NSUInteger number = [self.pageSwitchViewDelegate numberOfTab:self];
    
    for (int i=0; i<number; i++) {
        UIViewController *vc = [self.pageSwitchViewDelegate pageSwitchView:self viewOfTab:i];
        [_viewArray addObject:vc];
        [_rootScrollView addSubview:vc.view];
    }
    [self createNameButtons];
    
    [_topScrollView setContentOffset:CGPointMake(10, 0)  animated:YES];
    
    DLog(@"%f",self.topScrollView.frame.origin.y);
    //选中第一个view
    if (self.pageSwitchViewDelegate && [self.pageSwitchViewDelegate respondsToSelector:@selector(pageSwitchView:didselectTab:)]) {
        [self.pageSwitchViewDelegate pageSwitchView:self didselectTab:_userSelectedChannelID - 100];
    }
...
- (void)createNameButtons
{
    ...
    int bWidth=(self.frame.size.width-kWidthOfButtonMargin*[_viewArray count])/[_viewArray count];
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        //设置按钮尺寸
        if (textSize.width>bWidth) {
            bWidth=textSize.width;
        }
        
        [button setFrame:CGRectMake(xOffset,0,
                                    bWidth, kHeightOfTopScrollView)];
        //计算下一个tab的x偏移量
        xOffset += bWidth + kWidthOfButtonMargin;
        
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 0, bWidth, _shadowImage.size.height);
            button.selected = YES;
        }
...
-(void)scrollHandlePan:(UIPanGestureRecognizer*) panParam
{
    ...
    if(_rootScrollView.contentOffset.x <= 0) {
        if (self.pageSwitchViewDelegate
            && [self.pageSwitchViewDelegate respondsToSelector:@selector(pageSwitchView:panLeftEdge:)]) {
            [self.pageSwitchViewDelegate pageSwitchView:self panLeftEdge:panParam];
        }
    } else if(_rootScrollView.contentOffset.x >= _rootScrollView.contentSize.width - _rootScrollView.bounds.size.width) {
        if (self.pageSwitchViewDelegate
            && [self.pageSwitchViewDelegate respondsToSelector:@selector(pageSwitchView:panRightEdge:)]) {
            [self.pageSwitchViewDelegate pageSwitchView:self panRightEdge:panParam];
        }
    }
}

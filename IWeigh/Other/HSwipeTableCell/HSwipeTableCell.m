...
@interface HSwipeTableInputOverlay : UIView
@property (nonatomic, weak) HSwipeTableCell * currentCell;
@end

@implementation HSwipeTableInputOverlay
...
@interface HSwipeButtonsView : UIView
@property (nonatomic, weak) HSwipeTableCell * cell;
@property (nonatomic, strong) UIColor * backgroundColorCopy;
@end

@implementation HSwipeButtonsView
{
    NSArray * _buttons;
    UIView * _container;
    BOOL _fromLeft;
    ...
    HSwipeExpansionLayout _expansionLayout;
...
-(instancetype) initWithButtons:(NSArray*) buttonsArray direction:(HSwipeDirection) direction differentWidth:(BOOL) differentWidth
{
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, containerWidth, maxSize.height)]) {
        _fromLeft = direction == HSwipeDirectionLeftToRight;
...
-(void) expandToOffset:(CGFloat) offset settings:(HSwipeExpansionSettings*) settings
{
    ...
    if (!_expandedButton) {
        ...
        [UIView animateWithDuration: duration animations:^{
            ...
            if (_expansionLayout == HSwipeExpansionLayoutCenter) {
...
#pragma clang diagnostic ignored "-Wundeclared-selector"
    if ([sender respondsToSelector:@selector(callHSwipeConvenienceCallback:)]) {
        //call convenience block callback if exits (usage of HSwipeButton class is not compulsory)
        autoHide = [sender performSelector:@selector(callHSwipeConvenienceCallback:) withObject:_cell];
    }
#pragma clang diagnostic pop
    ...
    if (_cell.delegate && [_cell.delegate respondsToSelector:@selector(swipeTableCell:tappedButtonAtIndex:direction:fromExpansion:)]) {
        ...
        autoHide|= [_cell.delegate swipeTableCell:_cell tappedButtonAtIndex:index direction:_fromLeft ? HSwipeDirectionLeftToRight : HSwipeDirectionRightToLeft fromExpansion:fromExpansion];
    }
...
-(void) transition:(HSwipeTransition) mode percent:(CGFloat) t
{
    switch (mode) {
        case HSwipeTransitionStatic: [self transitionStatic:t]; break;
        case HSwipeTransitionDrag: [self transitionDrag:t]; break;
        case HSwipeTransitionClipCenter: [self transitionClip:t]; break;
        case HSwipeTransitionBorder: [self transtitionFloatBorder:t]; break;
        case HSwipeTransitionRotate3D: [self transition3D:t]; break;
    }
...
@implementation HSwipeSettings
-(instancetype) init
{
    if (self = [super init]) {
        self.transition = HSwipeTransitionBorder;
        self.threshold = 0.5;
        self.offset = 0;
        self.keepButtonsSwiped = YES;
        self.showAnimation = [[HSwipeAnimation alloc] init];
        self.hideAnimation = [[HSwipeAnimation alloc] init];
        self.stretchAnimation = [[HSwipeAnimation alloc] init];
    }
    return self;
}
...
@implementation HSwipeExpansionSettings
-(instancetype) init
{
    if (self = [super init]) {
        self.buttonIndex = -1;
        ...
        self.triggerAnimation = [[HSwipeAnimation alloc] init];
    }
    return self;
}
@end

@interface HSwipeAnimationData : NSObject
...
@property (nonatomic, strong) HSwipeAnimation * animation;

@end

@implementation HSwipeAnimationData
@end


#pragma mark Easing Functions and HSwipeAnimation
...
@implementation HSwipeAnimation

-(instancetype) init {
    if (self = [super init]) {
        _duration = 0.3;
        _easingFunction = HSwipeEasingFunctionCubicOut;
    }
    return self;
}
...
-(CGFloat) value:(CGFloat)elapsed duration:(CGFloat)duration from:(CGFloat)from to:(CGFloat)to
{
    ...
    switch (_easingFunction) {
        case HSwipeEasingFunctionLinear: easingFunction = mgEaseLinear; break;
        case HSwipeEasingFunctionQuadIn: easingFunction = mgEaseInQuad;;break;
        case HSwipeEasingFunctionQuadOut: easingFunction = mgEaseOutQuad;;break;
        case HSwipeEasingFunctionQuadInOut: easingFunction = mgEaseInOutQuad;break;
        case HSwipeEasingFunctionCubicIn: easingFunction = mgEaseInCubic;break;
        default:
        case HSwipeEasingFunctionCubicOut: easingFunction = mgEaseOutCubic;break;
        case HSwipeEasingFunctionCubicInOut: easingFunction = mgEaseInOutCubic;break;
        case HSwipeEasingFunctionBounceIn: easingFunction = mgEaseInBounce;break;
        case HSwipeEasingFunctionBounceOut: easingFunction = mgEaseOutBounce;break;
        case HSwipeEasingFunctionBounceInOut: easingFunction = mgEaseInOutBounce;break;
    }
...
#pragma mark HSwipeTableCell Implementation


@implementation HSwipeTableCell
{
    ...
    HSwipeButtonsView * _leftView;
    HSwipeButtonsView * _rightView;
    bool _allowSwipeRightToLeft;
    bool _allowSwipeLeftToRight;
    __weak HSwipeButtonsView * _activeExpansion;
    
    HSwipeTableInputOverlay * _tableInputOverlay;
    ...
    HSwipeAnimationData * _animationData;
...
-(void) initViews: (BOOL) cleanButtons
{
    if (cleanButtons) {
        ...
        _leftSwipeSettings = [[HSwipeSettings alloc] init];
        _rightSwipeSettings = [[HSwipeSettings alloc] init];
        _leftExpansion = [[HSwipeExpansionSettings alloc] init];
        _rightExpansion = [[HSwipeExpansionSettings alloc] init];
    }
    _animationData = [[HSwipeAnimationData alloc] init];
...
-(void) fetchButtonsIfNeeded
{
    if (_leftButtons.count == 0 && _delegate && [_delegate respondsToSelector:@selector(swipeTableCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)]) {
        _leftButtons = [_delegate swipeTableCell:self swipeButtonsForDirection:HSwipeDirectionLeftToRight swipeSettings:_leftSwipeSettings expansionSettings:_leftExpansion];
    }
    if (_rightButtons.count == 0 && _delegate && [_delegate respondsToSelector:@selector(swipeTableCell:swipeButtonsForDirection:swipeSettings:expansionSettings:)]) {
        _rightButtons = [_delegate swipeTableCell:self swipeButtonsForDirection:HSwipeDirectionRightToLeft swipeSettings:_rightSwipeSettings expansionSettings:_rightExpansion];
    }
}
...
-(void) createSwipeViewIfNeeded
{
    ...
    if (!_leftView && _leftButtons.count > 0) {
        _leftView = [[HSwipeButtonsView alloc] initWithButtons:_leftButtons direction:HSwipeDirectionLeftToRight differentWidth:_allowsButtonsWithDifferentWidth];
    ...
    if (!_rightView && _rightButtons.count > 0) {
        _rightView = [[HSwipeButtonsView alloc] initWithButtons:_rightButtons direction:HSwipeDirectionRightToLeft differentWidth:_allowsButtonsWithDifferentWidth];
...
- (void) showSwipeOverlayIfNeeded
{
    ...
    if (!_allowsMultipleSwipe) {
        ...
        _tableInputOverlay = [[HSwipeTableInputOverlay alloc] initWithFrame:table.bounds];
...
-(void) prepareForReuse
{
    ...
    [self cleanViews];
    if (_swipeState != HSwipeStateNone) {
        _triggerStateChanges = YES;
        [self updateState:HSwipeStateNone];
    }
...
-(void) updateState: (HSwipeState) newState;
{
...
- (void)setSwipeOffset:(CGFloat) newOffset;
{
    ...
    HSwipeButtonsView * activeButtons = sign < 0 ? _rightView : _leftView;
    ...
    HSwipeButtonsView* but[2] = {_leftView, _rightView};
    HSwipeSettings* settings[2] = {_leftSwipeSettings, _rightSwipeSettings};
    HSwipeExpansionSettings * expansions[2] = {_leftExpansion, _rightExpansion};
    
    for (int i = 0; i< 2; ++i) {
        HSwipeButtonsView * view = but[i];
        if (!view) continue;
        ...
        if (expand) {
            ...
            [self updateState:i ? HSwipeStateExpandingRightToLeft : HSwipeStateExpandingLeftToRight];
        }
        else {
            ...
            [self updateState:i ? HSwipeStateSwipingRightToLeft : HSwipeStateSwipingLeftToRight];
        }
    }
}
...
{
    HSwipeAnimation * animation = animated ? (_swipeOffset > 0 ? _leftSwipeSettings.hideAnimation: _rightSwipeSettings.hideAnimation) : nil;
...
-(void) showSwipe: (HSwipeDirection) direction animated: (BOOL) animated completion:(void(^)()) completion
{
    ...
    UIView * buttonsView = direction == HSwipeDirectionLeftToRight ? _leftView : _rightView;
    
    if (buttonsView) {
        CGFloat s = direction == HSwipeDirectionLeftToRight ? 1.0 : -1.0;
        HSwipeAnimation * animation = animated ? (direction == HSwipeDirectionLeftToRight ? _leftSwipeSettings.showAnimation : _rightSwipeSettings.showAnimation) : nil;
...
{
    CGFloat s = direction == HSwipeDirectionLeftToRight ? 1.0 : -1.0;
    HSwipeExpansionSettings* expSetting = direction == HSwipeDirectionLeftToRight ? _leftExpansion : _rightExpansion;
    ...
    if(!_activeExpansion && expSetting.fillOnTrigger) {
        ...
        if (buttonsView) {
            __weak HSwipeButtonsView * expansionView = direction == HSwipeDirectionLeftToRight ? _leftView : _rightView;
            __weak HSwipeTableCell * weakself = self;
...
{
    HSwipeAnimation * animation = animated ? [[HSwipeAnimation alloc] init] : nil;
...
-(void) panHandler: (UIPanGestureRecognizer *)gesture
{
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        if (!_allowsMultipleSwipe) {
            ...
            for (HSwipeTableCell * cell in cells) {
                if ([cell isKindOfClass:[HSwipeTableCell class]] && cell != self) {
        ...
        HSwipeButtonsView * expansion = _activeExpansion;
        if (expansion) {
            UIView * expandedButton = [expansion getExpandedButton];
            HSwipeExpansionSettings * expSettings = _swipeOffset > 0 ? _leftExpansion : _rightExpansion;
            ...
            HSwipeSettings * settings = _swipeOffset > 0 ? _leftSwipeSettings : _rightSwipeSettings;
            HSwipeAnimation * animation = nil;
            if (_targetOffset == 0) {
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if (gestureRecognizer == _panRecognizer) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(swipeTableCell:canSwipe:fromPoint:)]) {
            ...
            _allowSwipeLeftToRight = [_delegate swipeTableCell:self canSwipe:HSwipeDirectionLeftToRight fromPoint:point];
            _allowSwipeRightToLeft = [_delegate swipeTableCell:self canSwipe:HSwipeDirectionRightToLeft fromPoint:point];
        }
        else if (_delegate && [_delegate respondsToSelector:@selector(swipeTableCell:canSwipe:)]) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
            _allowSwipeLeftToRight = [_delegate swipeTableCell:self canSwipe:HSwipeDirectionLeftToRight];
            _allowSwipeRightToLeft = [_delegate swipeTableCell:self canSwipe:HSwipeDirectionRightToLeft];

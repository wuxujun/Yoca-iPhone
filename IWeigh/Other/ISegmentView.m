...
@class ISegmentItem;
@protocol ISegmentItemDelegate
- (void)ItemStateChanged:(ISegmentItem *)item index:(NSInteger)index isSelected:(BOOL)isSelected;
@end

@interface ISegmentItem : UIView
...
- (void)setTintColor:(UIColor *)tintColor
{
    ...
    if (_tintColor != tintColor) {
        ...
        for (NSInteger i = 0; i<self.itemsArray.count; i++) {
            ISegmentItem *item = self.itemsArray[i];
...
#pragma mark - ISegmentItemDelegate
- (void)ItemStateChanged:(ISegmentItem *)currentItem index:(NSInteger)index isSelected:(BOOL)isSelected
{
    if (self.itemsArray.count <=2) {
        return;
    }

...
@implementation IScrollDigitView
...
- (void)setNumber:(NSUInteger)number withAnimationType:(IScrollNumAnimationType)type animationTime:(NSTimeInterval)timeSpan {
    for (int i = 0; i < numberSize; ++i) {
        IScrollDigitView *digitView = [_numberViews objectAtIndex:i];
        NSUInteger digit = [IScrollNumView digitFromNum:number withIndex:i];
        if (digit != [self digitIndex:i] || type == IScrollNumAnimationTypeRand)
            switch (type) {
                case IScrollNumAnimationTypeNone:
                    [digitView setDigit:digit from:digit];
                    break;
                    
                case IScrollNumAnimationTypeNormal:
                    [digitView setDigit:digit from:0];
                    break;
                case IScrollNumAnimationTypeFromLast:
                    [digitView setDigitFromLast:digit];
                    break;
                    
                case IScrollNumAnimationTypeRand:
                    [digitView setRandomScrollDigit:digit length:self.randomLength];
                    break;
                case IScrollNumAnimationTypeFast:
    ...
    for (IScrollDigitView *digitView in _numberViews) {
...
- (NSUInteger)digitIndex:(NSUInteger)index {
    return [IScrollNumView digitFromNum:self.numberValue withIndex:index];
...
- (void)didConfigFinish {
    ...
    for (int i = 0; i < numberSize; ++i) {
        ...
        IScrollDigitView *digitView = [[IScrollDigitView alloc] initWithFrame:rect];

...
#import "IValuePopupView.h"

@implementation CALayer (IAnimationAdditions)
...
NSString *const IFillColorAnimation = @"fillColor";

@implementation IValuePopUpView
{
    BOOL _shouldAnimate;
...
- (void)setColor:(UIColor *)color
{
    ...
    [_colorAnimLayer removeAnimationForKey:IFillColorAnimation]; // single color, no animation required
}
...
- (void)setAnimatedColors:(NSArray *)animatedColors withKeyTimes:(NSArray *)keyTimes
{
    ...
    CAKeyframeAnimation *colorAnim = [CAKeyframeAnimation animationWithKeyPath:IFillColorAnimation];
    ...
    [_colorAnimLayer addAnimation:colorAnim forKey:IFillColorAnimation];
}

- (void)setAnimationOffset:(CGFloat)animOffset returnColor:(void (^)(UIColor *opaqueReturnColor))block
{
    if ([_colorAnimLayer animationForKey:IFillColorAnimation]) {
